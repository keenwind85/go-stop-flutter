...
describe('handleFallback', () => {
    ...
    describe('Preview Model Fallback Logic', () => {
        ...
        it('should only set Preview Model bypass mode on retryable quota failure', async () => {
            ...
            const retryableQuotaError = new RetryableQuotaError('Capacity error', mockGoogleApiError, 5);
            await handleFallback(mockConfig, previewModel, AUTH_OAUTH, retryableQuotaError);
        ...
        it('should not set Preview Model bypass mode on non-retryable quota failure', async () => {
            ...
            const terminalQuotaError = new TerminalQuotaError('quota error', mockGoogleApiError, 5);
            await handleFallback(mockConfig, previewModel, AUTH_OAUTH, terminalQuotaError);
        ...
        it('should silently retry if Preview Model fallback mode is already active and error is retryable error', async () => {
            ...
            const retryableQuotaError = new RetryableQuotaError('Capacity error', mockGoogleApiError, 5);
            const result = await handleFallback(mockConfig, previewModel, AUTH_OAUTH, retryableQuotaError);
            expect(result).toBe(true);
            expect(mockHandler).not.toHaveBeenCalled();
        });
        ...
        it('should activate regular fallback when handler returns "retry_always" and is TerminalQuotaError', async () => {
            ...
            const result = await handleFallback(mockConfig, previewModel, AUTH_OAUTH, terminalError);
            expect(result).toBe(true);
        ...
        it('should NOT set fallback mode if user chooses "retry_once"', async () => {
            ...
            const result = await handleFallback(mockConfig, PREVIEW_GEMINI_MODEL, AuthType.LOGIN_WITH_GOOGLE, terminalQuotaError);
            expect(result).toBe(true);
            expect(mockConfig.setPreviewModelBypassMode).not.toHaveBeenCalled();
        ...
        it('should pass DEFAULT_GEMINI_MODEL as fallback when Preview Model fails with Retryable Error', async () => {
            ...
            const retryableQuotaError = new RetryableQuotaError('Capacity error', mockGoogleApiError, 5);
            await handleFallback(mockConfig, PREVIEW_GEMINI_MODEL, AuthType.LOGIN_WITH_GOOGLE, retryableQuotaError);
            expect(mockConfig.fallbackModelHandler).toHaveBeenCalledWith(PREVIEW_GEMINI_MODEL, DEFAULT_GEMINI_MODEL, retryableQuotaError);
        });
        ...
        it('should pass DEFAULT_GEMINI_FLASH_MODEL as fallback when Preview Model fails with other error', async () => {
            ...
            const terminalQuotaError = new TerminalQuotaError('quota error', mockGoogleApiError, 5);
            await handleFallback(mockConfig, PREVIEW_GEMINI_MODEL, AuthType.LOGIN_WITH_GOOGLE, terminalQuotaError);
            expect(mockConfig.fallbackModelHandler).toHaveBeenCalledWith(PREVIEW_GEMINI_MODEL, DEFAULT_GEMINI_FLASH_MODEL, terminalQuotaError);
        });
    });
